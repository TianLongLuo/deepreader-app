...
it('sends MiMo model, api-key and completion token limit without DeepSeek parameters',async()=>{
 ...
 expect(body.model).toBe('mimo-v2.6-pro');expect(body.max_completion_tokens).toBe(321);expect(body.max_tokens).toBeUndefined();expect(body.thinking).toBeUndefined();expect(body.response_format).toEqual({type:'json_object'});expect(result.provider).toBe('mimo');
});
